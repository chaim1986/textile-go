...
func handleMessages(_ *astilectron.Window, m bootstrap.MessageIn) (payload interface{}, err error) {
	switch m.Name {
	case "ipfs.peerId":
		// Temporary method used just for testing
		var content string
		...
		payload = "connected! PeerId is no longer a method"
	...
	case "ipfs.getPath":
		...
		fmt.Print(path)
		payload, err = textile.GetPhotoBase64String(path)
		if err != nil {
			payload = err.Error()
			return
		}
...
// Carson: Leaving this for now, might be really helpful for moving user's files to static directories
//
